...
with st.container():
    ...
    st.sidebar.markdown("<p style='font-size:24px,margin-bottom:150px'>User Input Features</p>", unsafe_allow_html=True)
    ...
    st.toast(f"{add_select_file} encours de selection")
    ...
    st.markdown('<li class="subtitle">Data source : <a href="https://www.expat-dakar.com/" target="_blank"> Expat Dakar<a/> --- <a href="https://dakar-auto.com/senegal" target="_blank"> Expat Dakar<a/></li>',unsafe_allow_html=True)
    ...
    if add_select_file =='Scrape Data beautifulSoup':
        ...
        with col2:
            ...
            if st.button('Vêtements Hommes'):
                data = vetement_homme.main_info_vetement(add_selectbox)
            ...
            if st.button('Chaussures Hommes'):
                data = chaussures_homme.main_info_chaussures_hommes(add_selectbox)
                rows, cols = data.shape
                st.markdown('<h5>Display data dimension</h5>',unsafe_allow_html=True)
                st.write(f'<p class="p-t">Data dimension: {rows} rows and {cols} columns.</p>',unsafe_allow_html=True)
                st.dataframe(data)
            ...
            if st.button('Vêtements Enfants'):
                data = vetement_enfant.main_info_vetements_enfants(add_selectbox)
                rows, cols = data.shape
                st.markdown('<h5>Display data dimension</h5>',unsafe_allow_html=True)
                st.write(f'<p class="p-t">Data dimension: {rows} rows and {cols} columns.</p>',unsafe_allow_html=True)
                st.dataframe(data)
            ...
            if st.button('Chaussures Enfants'):
                data = chaussure_enfant.main_info_chaussures_enfants(add_selectbox)
                rows, cols = data.shape
                st.markdown('<h5>Display data dimension</h5>',unsafe_allow_html=True)
                st.write(f'<p class="p-t">Data dimension: {rows} rows and {cols} columns.</p>',unsafe_allow_html=True)
                st.dataframe(data)
